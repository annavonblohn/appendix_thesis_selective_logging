...
const char *root_model[]={"no","linear","logistic"};
...
Bool fscanconfig(Config *config,    /**< LPJ configuration */
 ...
 const char *fdi[]={"nesterov","wvpd"};
  ...
  fscanbool2(file,&config->individual,"individual");
  config->isD95max=FALSE;
  config->inheritance=FALSE;
  config->logging=FALSE;
  if(fscanbool(file,&config->isD95max,"isD95max",TRUE,verbose))
    return TRUE;
  if(config->individual)
  {
    fscanint2(file,&config->npatch,"npatch");
 if(fscanbool(file,&config->inheritance,"inheritance",TRUE,verbose))
      return TRUE;
    if(fscanbool(file,&config->logging,"logging",TRUE,verbose))
      return TRUE;
    if(config->logging)
    {
       if(fscanlimit(file,&config->logging_wooddensity,"logging_wooddensity",verbose))
         return TRUE;
    }
  }
  else
  ...
  if(fscankeywords(file,&config->root_model,"root_model",root_model,3,FALSE,verbose))
    return TRUE;
 ...
 if(config->river_routing)
  ...
  fscanbool2(file,&config->istrack,"istrack");
  ...
  config->pft_est=FALSE;
  if(fscanbool(file,&config->pft_est,"pft_est",TRUE,verbose))
    return TRUE;
  ...
  fscanint2(file,&config->tree_year,"tree_year");
  fscanint2(file,&config->cut_year,"cut_year");
...
config->compress=0;
  ...
  scanclimatefilename(&input,&config->soildepth_filename,FALSE,FALSE,"soildepth");
  if(config->istrack)
  {
    scanclimatefilename(&input,&config->track_filename,FALSE,FALSE,"track");
    if(config->track_filename.fmt!=CLM)
    {
      if(verbose)
        fprintf(stderr,"ERROR241: File type of track file '%s' must be CLM.\n",config->track_filename.name);
      return TRUE;
    }
 if (config->logging)
  {
    scanclimatefilename(&input,&config->logging_filename,FALSE,TRUE,"logging");
  }
  if(config->withlanduse!=NO_LANDUSE)
  {
 ...
 if(grassfix == GRASS_FIXED_PFT)
  ...
  if(config->pft_est)
  {
    scanclimatefilename(&input,&config->pft_est_filename,FALSE,TRUE,"pft_est");
    config->pft_est_map=fscantreemap(file,&config->pft_est_map_size,"pft_est_map",config->npft[GRASS]+config->npft[TREE],config);
    if(config->pft_est_map==NULL)
      return TRUE;
  }
  else
    config->pft_est_map=NULL;
  if(config->river_routing)
  {
    ...
    if(config->withlanduse!=NO_LANDUSE)
    {
      ...
      if(config->aquifer_irrig==AQUIFER_IRRIG)
        scanclimatefilename(file,&config->aquifer_filename,FALSE,FALSE,"aquifer");
#endif
    }
  }
